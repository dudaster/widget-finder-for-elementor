...
class WFX_Plugin_Tracker {
	...
	public static function record_install( string $slug, string $plugin_name, ?string $plugin_file = null ): void {
		...
		wp_cache_delete( 'wfx_all_tracked_plugins', 'widget-finder-for-elementor' );
	}
	...
	public static function record_widget_types( string $slug ): void {
		...
		// wfx_plugin_map is populated from data/widgets.json at activation time.
		$ref_table = $wpdb->prefix . WFX_Importer::TABLE_MAP;
		$types     = $wpdb->get_col(
			$wpdb->prepare(
				'SELECT widget_key FROM ' . esc_sql( $ref_table ) . ' WHERE plugin_slug = %s',
				$slug
			)
		) ?: [];
	...
	public static function get_all(): array {
		$cached = wp_cache_get( 'wfx_all_tracked_plugins', 'widget-finder-for-elementor' );
		if ( false !== $cached ) {
			return $cached;
		}

		global $wpdb;
		$results = $wpdb->get_results( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- no user input; table name sanitised via esc_sql()
			'SELECT * FROM ' . esc_sql( $wpdb->prefix . self::TABLE_PLUGINS ) . ' ORDER BY installed_at DESC'
		) ?: [];

		wp_cache_set( 'wfx_all_tracked_plugins', $results, 'widget-finder-for-elementor' );
		return $results;
	}
	...
	public static function get( string $slug ): ?object {
		...
		return $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . esc_sql( $wpdb->prefix . self::TABLE_PLUGINS ) . ' WHERE plugin_slug = %s',
				$slug
			)
		) ?: null;
	}
	...
	public static function clear_usage_cache( string $slug ): void {
		delete_transient( 'wfx_usage_' . md5( $slug ) );
	}
	...
	public static function enforce_active_limit( int $max ): void {
		...
		foreach ( $all as $plugin ) {
			...
			if ( $info['status'] !== WFX_Plugin_Manager::STATUS_ACTIVE ) continue;
	...
	public static function enforce_inactive_limit( int $max ): void {
		...
		foreach ( $all as $plugin ) {
			$info = $statuses[ $plugin->plugin_slug ] ?? [ 'status' => 'not_installed', 'plugin_file' => null ];
			if ( $info['status'] !== WFX_Plugin_Manager::STATUS_INACTIVE ) continue;
			if ( self::get_usage_count( $plugin->plugin_slug ) > 0 ) continue;
	...
	private static function compute_usage_count( string $slug ): int {
		...
		$types = $wpdb->get_col(
			$wpdb->prepare(
				'SELECT widget_type FROM ' . esc_sql( $map_table ) . ' WHERE plugin_slug = %s',
				$slug
			)
		) ?: [];
		...
		// Run one query per widget type and collect distinct post IDs in PHP.
		...
		foreach ( $types as $type ) {
			...
			$like1   = '%"widgetType":"' . $escaped . '"%';
			$like2   = '%"widgetType": "' . $escaped . '"%';

			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value -- intentional: must search Elementor JSON stored in meta_value; results cached via transient in get_usage_count()
			$ids = $wpdb->get_col(
				$wpdb->prepare(
					'SELECT DISTINCT pm.post_id
					FROM ' . esc_sql( $wpdb->postmeta ) . ' pm
					INNER JOIN ' . esc_sql( $wpdb->postmeta ) . ' em
						ON  em.post_id    = pm.post_id
						AND em.meta_key   = %s
						AND em.meta_value = %s
					INNER JOIN ' . esc_sql( $wpdb->posts ) . ' p
						ON  p.ID         = pm.post_id
						AND p.post_type != %s
					WHERE pm.meta_key = %s
					AND  (pm.meta_value LIKE %s OR pm.meta_value LIKE %s)',
					'_elementor_edit_mode', 'builder', 'revision', '_elementor_data', $like1, $like2
				)
			) ?: [];
