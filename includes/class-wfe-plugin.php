...
class WFE_Plugin {
	...
	private static ?WFE_Plugin $instance = null;

	public static function instance(): WFE_Plugin {
	...
	private function init_components(): void {
		...
		if ( did_action( 'elementor/loaded' ) ) {
			...
			WFE_Elementor_Integration::init();
		} else {
			// Elementor not yet loaded — hook for when it fires
			add_action( 'elementor/loaded', [ WFE_Elementor_Integration::class, 'init' ] );
		}

		// Daily cron for auto-delete
		add_action( 'wfe_daily_cleanup', [ $this, 'run_auto_delete' ] );
		if ( ! wp_next_scheduled( 'wfe_daily_cleanup' ) ) {
			wp_schedule_event( time(), 'daily', 'wfe_daily_cleanup' );
		}
	}
	...
	public function run_auto_delete(): void {
		...
		if ( $s['max_active_unused'] > 0 ) {
			WFE_Plugin_Tracker::enforce_active_limit( $s['max_active_unused'] );
		}

		if ( $s['max_inactive_unused'] > 0 ) {
			WFE_Plugin_Tracker::enforce_inactive_limit( $s['max_inactive_unused'] );
		}
	}
}
