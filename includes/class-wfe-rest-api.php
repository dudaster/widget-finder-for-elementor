 ...
 *
 * Routes:
 *   GET  /wp-json/wfe/v1/search?q=term    — search widgets
 *   POST /wp-json/wfe/v1/install           — install a plugin
 *   POST /wp-json/wfe/v1/activate          — activate an installed plugin
 *   POST /wp-json/wfe/v1/deactivate        — deactivate a plugin
 *   POST /wp-json/wfe/v1/uninstall         — uninstall + remove from tracker
 */
class WFE_Rest_API {

	private static ?WFE_Rest_API $instance = null;

	public static function instance(): WFE_Rest_API {
	...
	public function register_routes(): void {
		register_rest_route( 'wfe/v1', '/search', [
		...
		register_rest_route( 'wfe/v1', '/install', [
		...
		register_rest_route( 'wfe/v1', '/activate', [
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'wfe/v1', '/deactivate', [
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'wfe/v1', '/debug', [
			'methods'             => WP_REST_Server::READABLE,
		...
		register_rest_route( 'wfe/v1', '/remove', [
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'wfe/v1', '/uninstall', [
			'methods'             => WP_REST_Server::CREATABLE,
	...
	public function handle_search( WP_REST_Request $request ): WP_REST_Response {
		...
		$rows = WFE_Database::search( $query, $limit + 1, $offset );
		...
		foreach ( $rows as $row ) {
			$status_info = $statuses[ $row->plugin_slug ] ?? [
				'status'      => WFE_Plugin_Manager::STATUS_NOT_INSTALLED,
				'plugin_file' => null,
			];
	...
	public function handle_install( WP_REST_Request $request ): WP_REST_Response {
		...
		$result      = WFE_Plugin_Manager::install( $slug );
		...
		$status = WFE_Plugin_Manager::get_status( $slug );

		// Record in tracker + snapshot widget-type mapping from WFE dataset
		WFE_Plugin_Tracker::record_install( $slug, $plugin_name ?: $slug, $status['plugin_file'] );
		WFE_Plugin_Tracker::record_widget_types( $slug );

		// Enforce inactive-unused limit: uninstall oldest if over the cap.
		$settings = WFE_Settings::get();
		if ( $settings['max_inactive_unused'] > 0 ) {
			WFE_Plugin_Tracker::enforce_inactive_limit( $settings['max_inactive_unused'] );
		}
	...
	public function handle_activate( WP_REST_Request $request ): WP_REST_Response {
		...
		$result      = WFE_Plugin_Manager::activate( $plugin_file );
		...
		if ( $settings['max_active_unused'] > 0 ) {
			WFE_Plugin_Tracker::enforce_active_limit( $settings['max_active_unused'] );
		}
	...
	public function handle_debug(): WP_REST_Response {
		...
		$map_rows = $wpdb->get_results( "SELECT plugin_slug, widget_type FROM {$map_table} ORDER BY plugin_slug, widget_type" ) ?: [];
		$map = [];
	...
	public function handle_remove( WP_REST_Request $request ): WP_REST_Response {
		...
		WFE_Plugin_Tracker::remove_widget_types( $slug );
		WFE_Plugin_Tracker::remove( $slug );
	...
	public function handle_uninstall( WP_REST_Request $request ): WP_REST_Response {
		...
		// Using delete_plugins() would call uninstall_plugin() first — if the
		// plugin's uninstall.php has a fatal error, WordPress's shutdown handler
		// returns a "critical error" JSON response and the files are never deleted.
		if ( $plugin_file ) {
			WP_Filesystem();
			...
			// Multi-file plugin: delete the entire plugin folder.
			// Single-file plugin (e.g. hello.php): delete just the file.
			...
			if ( ! $deleted ) {
				return new WP_REST_Response( [
					'success' => false,
					'message' => __( 'Could not delete plugin files. Check file permissions.', 'widget-finder' ),
				], 500 );
			}
		}
