 ...
 *   {prefix}wfx_widgets    — widget search data (plugin info + widget tuples)
 *   {prefix}wfx_plugin_map — canonical plugin → Elementor widget-key mapping
 *
 * ── JSON format v1 ───────────────────────────────────────────────────────────
 *
 *   {
...
class WFX_Importer {
	...
	const TABLE_WIDGETS = 'wfx_widgets';
	const TABLE_MAP     = 'wfx_plugin_map';
	...
	public static function maybe_import(): void {
		if ( get_option( 'wfx_data_version' ) === self::DATA_VERSION ) {
			return;
		}

		self::create_tables();
		self::run_import();
		update_option( 'wfx_data_version', self::DATA_VERSION, false );
	}

	// ── Import ─────────────────────────────────────────────────────────────────

	private static function run_import(): void {
		$file = WFX_PATH . 'data/widgets.json';
		if ( ! file_exists( $file ) ) {
			return;
		}
		...
		$wpdb->query( 'TRUNCATE TABLE ' . esc_sql( $w_table ) );
		$wpdb->query( 'TRUNCATE TABLE ' . esc_sql( $m_table ) );
		...
		self::batch_insert( $m_table, $map_rows, [ '%s', '%s' ] );
	}

	// ── Helpers ────────────────────────────────────────────────────────────────

	/**
	 * Inserts rows using $wpdb->insert() wrapped in a transaction for performance.
	 *
	 ...
	 */
	private static function batch_insert( string $table, array $rows, array $formats ): void {
		global $wpdb;
